...
impl PinnedMemoryPool {
    ...
    pub fn allocate<T: Sized>(&mut self, len: usize) -> *mut T {
        let size_in_bytes = mem::size_of::<T>().checked_mul(len).expect("Allocation size overflow");
        if size_in_bytes == 0 {
            ...
            if len > 0 && mem::size_of::<T>() == 0 { // ZSTs of non-zero length
        ...
        match self.pool.allocate(log_factor) {
            ...
            Err(e) => panic!("PinnedMemoryPool: allocation failed for log_factor {}: {}", log_factor, e),
        }
    }
    ...
    pub fn preallocate(&mut self, num_slabs: usize) {
        ...
        for _ in 0..num_slabs {
            match self.pool.allocate(self.max_log_factor) {
                ...
                Err(e) => {
                    ...
                    eprintln!("PinnedMemoryPool: preallocate failed to allocate a slab: {}", e);
        ...
        for ptr_nn in alloc_ptrs {
            if let Err(e) = self.pool.deallocate(ptr_nn) {
                eprintln!("PinnedMemoryPool: preallocate failed to deallocate a slab: {}", e);
    ...
    pub fn free<T: Sized>(&mut self, ptr: *mut T) {
        if ptr.is_null() {
            ...
            if mem::size_of::<T>() == 0 { // For ZSTs, ptr might be dangling. Freeing it is a no-op.
                return;
            }
...
mod tests {
    ...
    fn test_memory_pool_pinned_wrapper() {
        ...
        let mut pool = PinnedMemoryPool::new(max_log, base_s);
        ...
        for iter_num in 0..iters {
            ...
            for item_num in 0..items_per_iter {
                ...
                if mem::size_of::<u32>() == 0 && len > 0 { // ZST case
                    let ptr = pool.allocate::<u32>(len);
                    assert!(!ptr.is_null(), "ZST allocation should return non-null (dangling) pointer");
                    ...
                    continue;
                } else if len == 0 { // Zero length non-ZST
                ...
                let ptr = pool.allocate::<u32>(len);
                assert!(!ptr.is_null(), "Allocation failed for iter {}, item {}, len {}", iter_num, item_num, len);
                allocations.push((ptr, len));
            ...
            for (ptr, len) in &allocations {
                if mem::size_of::<u32>() == 0 && *len > 0 { // ZST
    ...
    fn test_preallocate_pinned() {
        let mut pool = PinnedMemoryPool::new(10, mem::size_of::<u32>());
    ...
    fn test_allocate_zero_len_non_zst() {
        let mut pool = PinnedMemoryPool::new(5, mem::size_of::<u32>());
