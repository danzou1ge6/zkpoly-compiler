...
impl MemoryPool {
    ...
    fn allocate_pinned_memory(&mut self, size: usize) -> Result<NonNull<c_void>, String> {
        ...
        if err == cudaError_cudaSuccess {
            // CUDA_SUCCESS is typically 0
            NonNull::new(ptr).ok_or_else(|| {
                "cudaMallocHost reported success but returned null pointer".to_string()
            })
        } else {
    ...
    fn free_pinned_memory(&mut self, ptr: NonNull<c_void>) -> Result<(), String> {
    ...
    fn split_slab(&mut self, parent_log_factor: u32) -> Result<(usize, usize), String> {
        ...
        if parent_log_factor > self.max_log_factor {
            return Err(format!(
                "Cannot split from log_factor {} which is > max_log_factor {}",
        ...
        let parent_location_opt = self.slabs[parent_slab_idx]
            .location
            .as_ref()
            .map(|ptr| *ptr);
        if let Some(parent_ptr) = parent_location_opt {
            self.slabs[lchild_idx].location = Some(parent_ptr);
            ...
            self.slabs[rchild_idx].location = Some(rchild_ptr);
        } else {
            // Parent slab has no memory location.
            return Err(format!(
                "Parent slab {} at log_factor {} has no memory location to split",
                parent_slab_idx, parent_log_factor
            ));
        }
    ...
    pub fn allocate(&mut self, log_factor: u32) -> Result<NonNull<c_void>, String> {
        if log_factor > self.max_log_factor {
            return Err(format!(
                "Requested log_factor {} exceeds max_log_factor {}",
                log_factor, self.max_log_factor
            ));
        }
        ...
        let ptr = match &self.slabs[slab_idx].location {
            ...
            None => {
                return Err(format!(
                    "Slab {} (log_factor {}) selected for allocation has no memory location",
                    slab_idx, log_factor
                ));
            }
        };
    ...
    fn try_merge(&mut self, parent_slab_idx: usize) {
        let (lchild_idx_opt, rchild_idx_opt, parent_log_factor, grandparent_idx_opt) = {
            ...
            if parent_slab.lchild.is_none() || parent_slab.rchild.is_none() {
                // Already merged or not split
                return;
            }
            (
                parent_slab.lchild,
                parent_slab.rchild,
                parent_slab.log_factor,
                parent_slab.parent,
            )
        };
    ...
    pub fn deallocate(&mut self, ptr: NonNull<c_void>) -> Result<(), String> {
        let slab_idx = self.active_allocations.remove(&ptr).ok_or_else(|| {
            "Pointer not found in active allocations or already deallocated".to_string()
        })?;
        ...
        if let Some(parent_idx) = parent_idx_opt {
            ...
            if log_factor < self.max_log_factor {
                // Redundant if parent_idx implies it's not a root max_log_factor slab
    ...
    pub fn clear(&mut self) -> Result<(), String> {
        ...
        for i in 0..self.slabs.len() {
            if self.slabs[i].log_factor == self.max_log_factor {
                if let Some(ptr) = self.slabs[i].location.take() {
                    if self.use_mmap {
                        let _ = self.slabs[i].mmap_handle.take();
                    } else {
                        // take to avoid borrow issues
                        if let Err(e) = self.free_pinned_memory(ptr) {
                            errors.push(format!(
                                "Clear: Failed to free memory for slab {}: {}",
                                i, e
                            ));
        ...
        self.slab_layers
            .iter_mut()
            .for_each(|layer| *layer = SlabLayerInfo::default());
    ...
    pub fn shrink(&mut self) -> Result<(), String> {
        ...
        if let Some(layer_info) = self.slab_layers.get(self.max_log_factor as usize) {
            ...
            while let Some(current_idx) = current_idx_opt {
                if !visited_indices.insert(current_idx) {
                    errors.push(format!(
                        "Shrink (Phase 1): Loop detected in slab list at index {}.",
                        current_idx
                    ));
                    break;
                }
                ...
                if current_idx >= self.slabs.len() {
                    errors.push(format!(
                        "Shrink (Phase 1): Invalid slab index {} encountered.",
                        current_idx
                    ));
                    break;
                }
                ...
                if slab.free && slab.log_factor == self.max_log_factor {
                    if let Some(ptr) = slab.location {
                        candidate_slabs_for_freeing.push((current_idx, ptr));
                    }
                }

                if next_idx_opt == layer_info.head_all_idx {
                    break;
                }
        ...
        for (slab_idx, ptr_to_free) in candidate_slabs_for_freeing {
            ...
            if self.use_mmap {
                let _ = self.slabs[slab_idx].mmap_handle.take();
                successfully_freed_indices.push(slab_idx);
            } else {
                if let Err(e) = self.free_pinned_memory(ptr_to_free) {
                    errors.push(format!(
                        "Shrink (Phase 2): Failed to free memory for slab {}: {}",
                        slab_idx, e
                    ));
        ...
        for slab_idx in successfully_freed_indices {
            ...
            if slab_idx < self.slabs.len() {
                ...
                errors.push(format!(
                    "Shrink (Phase 3): Invalid slab index {} during metadata update.",
                    slab_idx
                ));
            }
        }
        ...
        for slab_idx in &to_remove_from_lists_final {
            ...
            if *slab_idx < self.slabs.len() && self.slabs[*slab_idx].free {
                ...
                errors.push(format!(
                    "Shrink (Phase 4): Invalid slab index {} during free list removal.",
                    slab_idx
                ));
                continue;
            }
        ...
        for slab_idx in to_release_info_final {
            // Consumes the vec
            if slab_idx < self.slabs.len() {
                // Check before releasing, though it should be fine if added correctly
                self.release_slab_info_index(slab_idx);
            } else {
                errors.push(format!(
                    "Shrink (Phase 4): Invalid slab index {} during info release.",
                    slab_idx
                ));
            }
        }
