...
fn test_pool_creation() {
    ...
    assert!(
        result.is_ok(),
        "Should be able to allocate at max_log_factor"
    );
    if let Ok(ptr) = result {
        // Deallocate to clean up
        pool.deallocate(ptr)
            .expect("Deallocation failed in pool creation test");
    }
}
...
fn test_simple_alloc_and_free_different_sizes() {
    ...
    for i in 1..=5 {
        // log_factor from 1 to 5
        let ptr_nn = pool
            .allocate(i)
            .expect(&format!("Failed to allocate for log_factor {}", i));
    ...
    for ptr_nn in allocations.into_iter().rev() {
        pool.deallocate(ptr_nn)
            .expect("Failed to deallocate in different sizes test");
    }
}
...
fn test_split_and_merge() {
    ...
    assert_eq!(
        ptr1_nn.as_ptr(),
        ptr3_nn.as_ptr(),
        "ptr3 should reuse the memory of ptr1 after merge"
    );
...
fn test_shrink_and_clear_usage() {
    ...
    for i in 1..=5 {
        ...
        let ptr_nn = pool
            .allocate(i)
            .expect(&format!("Allocation failed for log_factor {}", i));
            ...
            unsafe {
                for j in 0..num_elements {
                    assert_eq!(
                        *current_ptr.add(j),
                        j as i32,
                        "Data corruption after shrink for log_factor {}",
                        i
                    );
                }
            }
            pool.deallocate(ptr_nn)
                .expect(&format!("Deallocation failed for log_factor {}", i));
        }
    }
    ...
    pool.deallocate(ptr_after_shrink_nn)
        .expect("Deallocate after shrink failed");

    pool.clear().expect("Clear failed");

    for i in 1..=5 {
        let ptr_nn = pool.allocate(i).expect(&format!(
            "Allocation failed for log_factor {} after clear",
            i
        ));
        let current_ptr = ptr_nn.as_ptr().cast::<i32>();
        let num_elements = 1 << i;
        unsafe {
            ...
            for j in 0..num_elements {
                assert_eq!(
                    *current_ptr.add(j),
                    j as i32,
                    "Data check failed for log_factor {} after clear",
                    i
                );
            }
        }
        pool.deallocate(ptr_nn)
            .expect("Deallocate after clear failed");
    }
}
...
fn test_complex() {
    ...
    for _k in 0..rounds {
        ...
        for _i in 0..items_per_round {
            ...
            let ptr_nn = pool.allocate(log_factor).expect(&format!(
                "Complex: Allocate failed for log_factor {}",
                log_factor
            ));
            ptrs_nn.push(ptr_nn);
        ...
        for &idx_to_dealloc in &indices {
            ...
            pool.deallocate(ptr_to_dealloc)
