...
impl<'s, Rt: RuntimeType> Cg<'s, Rt> {
    pub fn precompute(
        ...
        let order = self.g.dfs().map(|(id, _)| id).collect::<Vec<_>>();
        ...
        for id in order.iter() {
            let vertex = self.g.vertex(*id).clone();
            match vertex.node() {
                super::template::VertexNode::Ntt { s, to, from, alg } => {
                    ...
                    if old_alg.is_some() {
                        *self.g.vertex_mut(*id).node_mut().unwrap_ntt_alg_mut() =
                        ...
                        match &mut alg {
                            NttAlgorithm::Precomputed(omegas_id) => {
                                ...
                                let load_c = self.g.add_vertex(Vertex::new(
                            ...
                            NttAlgorithm::Standard {
                                ...
                                let mut omegas =
                                    ScalarArray::alloc_cpu(recompute_omega_len, allocator);
                                ...
                                let load_pq = self.g.add_vertex(Vertex::new(
                                ...
                                let load_omegas = self.g.add_vertex(Vertex::new(
                        ...
                        *self.g.vertex_mut(*id).node_mut().unwrap_ntt_alg_mut() = alg.clone();
                ...
                super::template::VertexNode::Msm { points, .. } => {
                    ...
                    if precompute.is_some() {
                        let (_, points, old_config) =
                            self.g.vertex_mut(*id).node_mut().unwrap_msm();
                        ...
                        *old_config = config;
                        ...
                        let input_constant_id = self.g.vertex(input_id).node().unwrap_constant();
                        let input_points =
                        ...
                        for c_id in c_ids {
                            let load_id = self.g.add_vertex(Vertex::new(
                                VertexNode::Constant(c_id),
                                self.g.vertex(input_id).typ().clone(),
                                self.g.vertex(input_id).src().clone(),
                            ));
